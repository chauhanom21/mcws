...
@Controller
@RequestMapping("/config/chartSpl")
public class ChartSplConfigController {
	...
	@ResponseBody
	public Response<String> saveUserClientDetails(@RequestBody JSONObject requestData) {
		Response<String> response = ResponseBuilder.buildOkResponse();
		try {
			List<Integer> clientIds = new ArrayList<Integer> ();
			Integer userId = iterateUserClientData(clientIds, requestData);
			
			if( userId > 0 && clientIds.size() > 0 ) {
	 ...
	 * @param requestParam
	 * 
	 * It Iterate the json data and assign to userId and clientIds List
	 */
	private Integer iterateUserClientData(List<Integer> clientIds, JSONObject requestParam) {
		...
		if(map.size() > 0) {
			...
			ArrayList<Integer> clientIdList = (ArrayList<Integer>) map.get("clientIds");
